...
Fatte queste premesse, non si può non considerare la cardiotografia computerizzata CTGc vista la posizione di primaria importanza che occupa nel fornire l'ausilio al monitoraggio della salute del feto. Essa infatti è stata introdotta con l’obiettivo di ridurre i falsi positivi e la soggettività dell’interpretazione poichè fornisce un’interpretazione oggettiva della CTG consentendo la comunicazione di numeri piuttosto che di opinioni. Negli anni infatti ci si è resi conto che i risultati pratici conseguiti dall’uso «di massa» della cardiotocografia sono stati di gran lunga inferiori alle aspettative a causa della carenza di criteri univoci di interpretazione e alla scarsa preparazione degli operatori sanitari. <br><br>
...
```
Vediamo dalla confusion matrix che il numero di casi patologici e dubbiosi classificati male si è ridotto e anche il rateo di patologici classificato come normali è calato drasticamente cosa che ci rassicura . 


```{r,  echo = F}
SMOTE_Confusion$byClass[,1:2]
```
Notiamo un aumento della sensitivity del caso patologico da 0.7 a 0.8 e un aumento della sensitivity del caso sospetto da 0.45 a 0.68, per cui è evidente che siamo più accurati nell'identificare i casi abnormali, altresì marcato è una diminuzione della sensitivity dei casi normali.
Il fondamentale trade-off che si fa quando utilizziamo tecniche di oversampling/undersampling è ridurre la sensitivity della classe maggioritaria per  aumentare le sensitivities delle classi minoritarie (dunque aumentando anche la specificity della maggioritaria).
In altre parole se poniamo come H0 : "essere di classe maggioritaria" , quello che facciamo è ridurre l'errore di primo tipo e aumentare l'errore di secondo
