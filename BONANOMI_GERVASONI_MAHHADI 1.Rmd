...
### 2) Materiali
I dati di interesse provengono dal dataset **fetal_health**, esportabile gratuitamente dal link citato a fine paragrafo. La raccolta di questi dati è stata fatta mediante SisPorto, un programma di analisi automatizzata del tracciato cardiotografico che individua e calcola una serie di metriche che vengono valutate durante il monitoraggio cardiotografico del feto, così come indicato dalla FIGO (Federazione Internazionale di Ginecologia e Ostetricia).
Grazie a queste rilevazioni si è ottenuto un dataset con 2126 osservazioni di cardiotografie fetali suddivise per 21 variabili a cui si somma l'etichetta di classe *fetal_health* rappresentata dalle seguenti modalità: 1,2,3 che indicano rispettivamente uno stato di salute del feto Normale, Sospetto, Patologico. Per completezza di informazione segue una tabella che riporta sinteticamente la descrizione e le modalità dell variabili:


VARIABILE     | DESCRIZIONE  | MODALITÀ 
------------- | -------------| ---------
baseline_value| Frequenza cardiaca fetale di base (FHR) |  Bpm
accelerations|Numero di accelerazioni cardiache al secondo | Hz
fetal_movement|Numero di accelerazioni fetali al secondo| Hz                
uterine_contractions| Numero di contrazioni uterine al secondo| Hz
light_decelerations|Numero di decelerazioni leggere al secondo|Hz            
severe_decelerations|Numero di decelerazioni gravi al secondo|Hz          
prolongued_decelerations|Numero di decelerazioni prolungate al secondo|Hz |    
abnormal_short_term_variability|Percentuale di tempo con variabilità a breve termine anomala|Percentuale|
mean_value_of_short_term_variability| Media della variabilità a breve termine|Millisecondi|  
percentage_of_time_with_abnormal_long_term...| Percentuale di tempo con variabilità a lungo termine anomala | Percentuale|                      
mean_value_of_long_term_variability| Media della variabilità a lungo termine| Millisecondi|   
histogram_width| Larghezza dell'istogramma della FHR| Bpm|                       
histogram_min| Valore minimo dell'istogramma della FHR| Bpm|                      
histogram_max| Valore massimo dell'istogramma della FHR| Bpm| 
...
histogram_mode | Moda dell'istogramma della FHR | Bpm|
histogram_mean| Valore medio dell'istogramma della FHR| Bpm|             
histogram_median| Mediana dell'istogramma della FHR| Bpm |               
histogram_variance| Varianza dell'istogramma della FHR| Bpm^2|
...
le ultime 2 variabili selezionate ("mean_value_of_long_term_variability" e "uterine_contractions")
...
```
Notiamo un aumento della sensitivity del caso patologico da 0.7 a 0.77 e un aumento della sensitivity del caso sospetto da 0.45 a 0.84, per cui è evidente che siamo più accurati nell'identificare un caso abnormali, altresì marcato è una diminuzione della sensitivity dei casi normali.
