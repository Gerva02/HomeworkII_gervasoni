...
```
Dopodichè abbiamo ripetuto il ggpairs sulle sole variabili selezionate. Notiamo che *mean_value_of_long_term_variability* e *uterine_contractions* non rispettano la normalità e la simmetria in tutti i gruppi essendo dati reali. 
...
In particolare, il gruppo dei sospetti viene completamente incluso all'interno dei normali e viene creata una nuova classe a noi sconosciuta che si colloca tra sani e i paologici. Quest'ultimo risulta essere, al netto di poche unità statistiche lontane dal baricentro, l'unico gruppo evidenziato correttamente. 
...
Al contrario per quanto riguarda l'entropia (356) ed l'entropia relativa (0.15) possiamo dedurre che il modello risulta affidabile.
Riteniamo opportuno confrontare i modelli VEV,7 ed EVV,3 che sono per distacco i migliori. 
Il modello con 7 gruppi in termini di bontà risulta migliore e lo si denota dal fatto che sia R^2 calcolato tramite determinante (0.89) che quello calcolato tramite traccia (0.39) sono superiori. 
Al contrario in termini di allocazione delle unità statistiche si denota un peggioramento osservabile dalla presenza di incertezza maggiore in ciascuna unità statistica e da un'entropia assoluta (1019) e relativa (0.25).
```{r,echo=FALSE}
