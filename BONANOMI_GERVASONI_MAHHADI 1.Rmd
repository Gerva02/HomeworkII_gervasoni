...
ggplot(aes(x=fetal_health,
  ...
  scale_fill_manual(values = c("Normale" = "#66FF66", "Sospetto" = "#00008B", "Patologico" = "red"))
```
...
fetal_Health %>%
  ...
  geom_point() +
  scale_color_manual(values = c("Normale" = "#66FF66", "Sospetto" = "#00008B", "Patologico" = "red"))
...
grafico1<-coordProj (as.data.frame(fetal_Health_EM), dimens=c(1,2), what="classification",
           ...
           col=c("red","#00008B","#66FF66"), symbols=c(0 ,16 ,17),
...
grafico2<-coordProj (data=as.data.frame(fetal_Health_EM), dimens=c(1,2), what="uncertainty",
           parameters=health_mclust_BIC_k3$parameters , z=health_mclust_BIC_k3$z, ylim=c(130,211),xlim=c(90,165),colors=c("red","#00008B","#66FF66")) + title("Grafico incertezze")
...
le classi minoritarie, identificando vicini prossimi e creando combinazioni
lineari tra le osservazioni esistenti. Questo processo migliora la rappresentazione
della classe meno frequente nel dataset, aiutando i modelli di classificazione a
generalizzare meglio durante l'addestramento. 
...
Nel nostro caso applichiamo l'algoritmo smote per ottenere un totale di 600 osservazioni tra dati sintetici e reali, per ogni classe minoritaria. Inoltre utilizziamo il default di 
